...
int main(int argc, char** argv){
    ...
    if (client.call(srv))
    {
        ROS_INFO("Localization request sent!!");
    }
    else
    {
        ROS_ERROR("Failed to request the global localization");
        return 2;
    }
    ...
    std::cout << target_pose_list.size() << "\n";
    move_base_msgs::MoveBaseGoal goal;

    no_goal = target_pose_list.size();

    for (size_t i=0; no_goal; i++)
    {
